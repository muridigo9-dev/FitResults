-- Migration: 20260130182500_update_daily_summary_body_profile.sql
-- Description: Update get_daily_summary RPC to include baseline health metrics from user_body_profiles
-- Created: 2026-01-30
                    ...
                    'exercises', (
                        SELECT COALESCE(jsonb_agg(
                            jsonb_build_object(
                                'id', e_inner.id,
                                'name', e_inner.name,
                                'sets', (SELECT COUNT(*) FROM public.session_sets ss WHERE ss.session_exercise_id = se_inner.id),
