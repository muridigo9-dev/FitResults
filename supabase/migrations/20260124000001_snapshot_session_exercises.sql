-- Update start_workout_session to snapshot exercise details
-- This ensures that if the original workout structure changes, the active session remains consistent.
-- Snapshot data is stored in session_exercises.metadata column.
...
BEGIN
  ...
  ELSE
    ...
    -- Assumes workout_exercises has sets, reps, rest_time columns
    FOR v_exercise IN
      SELECT 
        we.id, 
        we.name,
