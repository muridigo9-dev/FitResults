  ...
  ELSE
    ...
    FOR v_exercise IN
      SELECT 
        we.id, 
        we.name,
        we.exercise_id, -- Selecionar o ID do exercício vinculado
        we.sets,
        we.reps,
        we.rest_seconds,
        we.exercise_order,
    ...
    LOOP
      ...
      -- 1. Tentar usar o link direto primeiro
      v_exercise_id := v_exercise.exercise_id;

      -- 2. Se não tiver link direto, tentar encontrar por nome (apenas se nome existir)
      ...
      -- 3. Se ainda nulo e tiver nome, criar novo exercício
      IF v_exercise_id IS NULL AND v_exercise.name IS NOT NULL THEN
      ...
      -- Só inserir na sessão se tivermos um ID de exercício válido
      ...
      END IF;
      -- Se v_exercise_id for NULL, pulamos este item para evitar erro de constraint, 
      -- mas o ideal seria garantir integridade dos dados na origem.
    END LOOP;
  END IF;
