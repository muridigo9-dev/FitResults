-- ============================================================
-- FIX DAILY SUMMARY FOR USER WORKOUTS
-- ============================================================
        ...
        'workouts', CASE WHEN v_show_workouts THEN (
            SELECT COALESCE(jsonb_agg(
                jsonb_build_object(
                    ...
                    'title', COALESCE(
                        w.title, 
                        uw.title, -- Priorities: System Workout -> User Workout -> First Exercise -> 'Treino'
                    ...
                    'isAdhoc', (ws.workout_id IS NULL AND ws.user_workout_id IS NULL), -- Is Adhoc ONLY if both are null
                    'exercises', (
                        SELECT COALESCE(jsonb_agg(
                            jsonb_build_object(
                                'id', e.id,
            ...
            LEFT JOIN public.user_workouts uw ON uw.id = ws.user_workout_id -- Added JOIN
