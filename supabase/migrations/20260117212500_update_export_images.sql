-- =========================================================
-- FIX: Update export_workouts to include Exercise Images
-- =========================================================
...
BEGIN
    ...
    raw_exercises AS (
        SELECT 
            ...
            e.equipment,
            e.image_url as exercise_image, -- ADICIONADO AQUI
            ...
            t.name as type_name, -- Mudado para name para exibição direta
            l.name as level_name -- Mudado para name para exibição direta
        ...
        SELECT 
            ...
            jsonb_agg(
                jsonb_build_object(
                    ...
                    'image_url', exercise_image, -- Incluindo no JSON do exercício
    ...
    -- 2.1 Calcular sequência dos blocos
    blocks_ordered AS (
        SELECT 
            workout_id,
            block_type,
