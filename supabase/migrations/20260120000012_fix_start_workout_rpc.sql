-- Fix start_workout_session RPC to use correct rest_seconds column
-- This fixes the error: column we.rest_time does not exist
  ...
  ELSE
    ...
    FOR v_exercise IN
      SELECT 
        ...
        we.sets,
        we.reps,
        we.rest_seconds, -- Fixed: Was rest_time
        we.exercise_order
      ...
      VALUES (
        ...
        jsonb_build_object(
          'snapshot', jsonb_build_object(
            'sets', v_exercise.sets,
            'reps', v_exercise.reps,
            'rest_seconds', v_exercise.rest_seconds, -- Fixed: Was rest_time
            'tempo', NULL,
