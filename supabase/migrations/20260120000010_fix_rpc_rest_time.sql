-- Fix start_workout_session RPC to use correct column name (rest_seconds) AND include superset_id
...
BEGIN
  ...
  ELSE
    ...
    -- FIX: Changed rest_time to rest_seconds AND added superset_id
    FOR v_exercise IN
      SELECT 
        we.id, 
        we.name,
        we.sets,
        we.reps,
        we.rest_seconds, -- FIXED here
        we.superset_id,  -- ADDED here
        we.exercise_order
      ...
      VALUES (
        ...
        jsonb_build_object(
          'snapshot', jsonb_build_object(
            'sets', v_exercise.sets,
            'reps', v_exercise.reps,
            'rest_seconds', v_exercise.rest_seconds, -- FIXED here
            'superset_id', v_exercise.superset_id,   -- ADDED here
            'tempo', NULL,
