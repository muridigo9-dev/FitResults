-- ================================================
-- EMAIL SYSTEM SCHEMA
-- Centralized email configuration, templates, and logs
-- ================================================

-- ==========================================
-- 1. EMAIL SETTINGS TABLE (Resend API Config)
-- ==========================================
CREATE TABLE IF NOT EXISTS public.email_settings (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    api_key_encrypted text, -- Encrypted Resend API key
    api_key_hint text, -- Last 4 chars for display (e.g., "...abc1")
    sender_name text NOT NULL DEFAULT 'App',
    sender_email text NOT NULL DEFAULT 'onboarding@resend.dev',
    is_enabled boolean NOT NULL DEFAULT false,
    test_email_address text, -- Admin email for testing
...
-- RLS for email_settings (Admin only)
...
CREATE TABLE IF NOT EXISTS public.email_templates (
    ...
    variables jsonb DEFAULT '[]'::jsonb, -- Available variables: ["user_name", "app_name", etc.]
    ...
    CONSTRAINT unique_active_template_type UNIQUE (type, is_active)
);

-- RLS for email_templates (Admin only)
...
-- RLS for email_logs (Admin read-only)
...
-- Add new enum values for payment templates
DO $$ BEGIN
...
VALUES 
    ...
    (
        'Pagamento Falhou',
        'payment_failed',
        '⚠️ Problema com seu pagamento - {{app_name}}',
        '<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f9fafb;">
            <div style="background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%); padding: 30px; border-radius: 12px 12px 0 0; text-align: center;">
                <h1 style="color: #ffffff; margin: 0; font-size: 24px;">⚠️ Houve um problema com seu pagamento</h1>
            </div>
            <div style="background: #ffffff; padding: 30px; border-radius: 0 0 12px 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
    ...
    (
        'Resposta do Suporte',
        'support_reply',
        'Resposta do suporte - {{ticket_subject}}',
        '<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            <h1 style="color: #333;">Olá, {{user_name}}</h1>
            ...
            <p>Se precisar de mais ajuda, responda este e-mail.</p>
            <p>Atenciosamente,<br>Equipe de Suporte {{app_name}}</p>
        </div>',
        'Olá {{user_name}}, aqui está nossa resposta: {{support_reply}}',
        '["user_name", "app_name", "user_email", "ticket_subject", "support_reply"]'::jsonb
    ),
    (
        'Notificação Geral',
        'general_notification',
        ...
        'Olá {{user_name}}, {{content}}',
        '["user_name", "app_name", "user_email", "subject", "content"]'::jsonb
    )
